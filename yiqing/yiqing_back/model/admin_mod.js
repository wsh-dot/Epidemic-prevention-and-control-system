    ...
    */
    ...
    static getreadByidArr(readIdArr) {
        ...
        for (let i = 0; i < readIdArr.length; i++) {
            if (i == 0) sql += "select id,username,head,classes,createtime from `user` where id = " + readIdArr[i].u_id
            else sql += " or id= " + readIdArr[i].u_id
        }
        return new Promise((resolve, reject) => {
            this.query(sql).then(result => {
                resolve(result)
    ...
    static NoticeDetailsTotal(classes) {
        ...
        for (let i = 0; i < classesArr.length; i++) {
            if (i == 0) sql += "select count(1) as count from `user` where classes= '" + classesArr[i] + "'"
            else
